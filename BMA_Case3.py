...
"""
Block Matching Algorithm 3 (BMA 3) Animation

This Manim animation visualizes the advanced Block Matching Algorithm 3 for text 
narrative alignment, specifically implementing Case 2 (REF vs REF with Verbosity).
This version demonstrates verbosity-aware text comparison with comprehensive 
visual effects and enhanced algorithm sophistication.

The animation demonstrates:
1. Advanced text block visualization with enhanced sophisticated styling
2. Case 2 implementation: REF vs REF with verbosity considerations
3. Comprehensive visual effects including sophisticated animations
4. Enhanced interactive paragraph displays with improved color schemes
5. Verbosity-aware narrative line and mapping window techniques
6. Advanced animation timing and comprehensive visual feedback
7. Sophisticated heatmap visualizations and data representations
...
class FullConceptAnimation(Scene):
    """
    Main animation class for visualizing Block Matching Algorithm 3 (BMA 3).
    
    This advanced version demonstrates narrative line and mapping window concepts
    with verbosity considerations, implementing Case 2 (REF vs REF with Verbosity)
    and featuring the most comprehensive visual effects and algorithm sophistication.
    """

    def construct(self):
        """Main animation sequence orchestrating all BMA 3 visualization components."""
        ...
        # Move title to top edge for maximum screen space utilization
        self.play(title.animate.to_edge(UP*0.1))
        self.wait(0.5)

        # Add underline for sophisticated visual emphasis
        underline = Underline(title)
        self.play(Create(underline))
        self.wait(0.5)
 
        # Group title and underline, then shift to left for verbosity case specification
        group_title = VGroup(title, underline)
        self.play(group_title.animate.to_edge(LEFT*1.8))
        self.wait(0.5)

        # Add Case 2 specification with verbosity emphasis
        case_text = Text(": Case 2 (REF vs REF with Verbosity)", font_size=25)
        ...
        # ================== ADVANCED TEXT BLOCK CREATION ==================
        # Create reference and generated text blocks with most sophisticated styling
        # Reference text block with advanced styling for verbosity demonstration
        reference_box = Rectangle(
            width=3.0, height=3.6,
            stroke_color=WHITE,
            stroke_width=2,               # Enhanced stroke width for superior visibility
            fill_color=BLUE_E,           # Blue background for reference text
            fill_opacity=0.4             # Semi-transparent fill for professional appearance
        )
        reference_label = Text("Reference", font_size=24, color=WHITE)
        reference_label.next_to(reference_box, UP, buff=0.2)

        # Reference text content - comprehensive market scene narrative for verbosity analysis
        reference_lines = [
        ...
        # ================== COMPREHENSIVE EXPLANATORY TEXT SYSTEM ==================
        # Create advanced reusable function for generating sophisticated formatted text groups
        def create_text_group(header_text, explanation_lines, header_color=ORANGE):
            """
            Creates a comprehensively formatted text group with header, underline, and explanation content.
            
            Args:
                header_text: Title text (e.g., "WHAT?", "WHY?", "HOW?")
                explanation_lines: List of detailed explanation text lines
                header_color: Color for header and underline (default: ORANGE)
                
            Returns:
                VGroup containing sophisticatedly formatted header and explanation text
            """
        ...
        brevity_text = Text("Verbosity", font_size=18, color=WHITE)
        ...
        # ================== SOPHISTICATED VISUAL TRANSITIONS ==================
        # Create L-shaped arrow with dynamic entrance and comprehensive visual effects
        self.play(
            AnimationGroup(
                Create(line_vertical, run_time=0.7),
                Create(arrow_horizontal, run_time=0.7),
                # Add sophisticated indication effect to the generated box for enhanced user engagement
                Indicate(generated_box, scale_factor=1.1, color=GREEN_E)
            )
        )
        self.wait(0.5)

        # ================== VERBOSITY BOX TRANSITION ==================
        # Create verbosity demonstration by transitioning from generated to verbosity box
        # Fade out the generated box with sophisticated visual effects
        self.play(
      ...
      # Capture the final scaled dimensions of the reference box
        ...
        ref_height = reference_box.height * 1.6
        ...
        brevity_label = Text("Verbosity", font_size=24, color=WHITE)
        brevity_label.next_to(brevity_box, UP, buff=0.1)
        
        brevity_lines = [
            "The old market bell rings with a clear,", 
            "resonant sound. This ringing marks the", 
            "beginning of a busy market.Vendors open", 
            "their bright stalls in the busy square.", 
            "Meanwhile, the inviting smell of fresh", 
            "bread fills the air.A young seller", 
            "vociferously shouts out attractive good", 
            "deals. In response, curious people begin", 
            "to gather around. The steady ring of the ",
            "bell resounds with unwavering consistency.", 
            "This persistent sound effectively sets the", 
            "pace for the day.A wise old vendor stops by", 
            "his stall in a measured, deliberate manner.", 
            "He gives advice to those who pass by.As the", 
            "market grows busy, the bell rings again at", 
            "midday. This ringing reminds everyone of", 
            "the community spirit. A light rain makes a", 
            "brief appearance that slows the movement of", 
            "the crowd. Despite this temporary pause,", 
            "everyone's spirit remains strong.Local", 
            "storytellers engage in the recitation of", 
            "simple tales. These uncomplicated narratives", 
            "manage to catch everyone's attention.As",
            "evening arrives, the old bell rings one", 
            "final time. Its sound perfectly echoes", 
       ...
       # Now display each segment
        ...
        lines_bottom_content = [
            "The old market bell rings with a clear resonant sound",
            "This ringing marks the beginning of a busy market",
            "Vendors open their bright stalls in the busy square",
            "Meanwhile the inviting smell of fresh bread fills the air",
            "........................................................",
            "........................................................",
            "........................................................",
            "As evening arrives the old bell rings one final time",
            "Its sound perfectly echoes the beginning of the day"
        ]

        # Define which positions should have ellipses
        ellipsis_positions = [4, 5, 6]  # 0-indexed positions that should have "........."

        # Define the last segment number (box 18)
        last_segment_number = 18
       ...
       # Now display each segment
        for j, chunk_content in enumerate(lines_bottom_content):
            ...
            # Add the number inside the box
            if j in ellipsis_positions:
                # No number for ellipsis positions
                number_text = Text("", font_size=10, color=WHITE)
            elif j == len(lines_bottom_content) - 1:
                # Use number 18 for the last box
                number_text = Text(f"{last_segment_number}", font_size=10, color=WHITE)
            elif j == len(lines_bottom_content) - 2:
                # Use number 17 for the second-to-last box
                number_text = Text(f"{last_segment_number-1}", font_size=10, color=WHITE)
            else:
                # Use sequential numbering for other boxes
        ...
        # Create a reference box and label for the top section
        fade_out_list = [
        ...
        # Truncate the sentences in the segments to make them shorter
        shorter_lines_top_mobs = []
        for i, line_mob in enumerate(lines_top_mobs):
            # Get the numbered box and text object
            number_box = line_mob[0]
            number_text = line_mob[1]
            text_obj = line_mob[2]
            
            # Get the current text and truncate it
            current_text = text_obj.text
            # Remove quotes and truncate to first 20 characters (or adjust as needed)
            truncated_text = f' "{current_text[2:22]}..."'
            
            # Create a new text object with truncated text
            new_text = Text(truncated_text, font_size=8.5)
            
            # Position the text closer to the box
            new_text.next_to(number_box, RIGHT, buff=0.1)  # Reduced buffer to 0.1 (was 0.2)
            
            # Create a new group with the original box, number, and new text
            new_line_mob = VGroup(number_box, number_text, new_text)
            shorter_lines_top_mobs.append(new_line_mob)

        shorter_lines_bottom_mobs = []
        for i, line_mob in enumerate(lines_bottom_mobs):
            # Get the numbered box and text object
            number_box = line_mob[0]
            number_text = line_mob[1]
            text_obj = line_mob[2]
            
            # For ellipsis lines, keep them as is
            if i in [4, 5, 6]:
                shorter_lines_bottom_mobs.append(line_mob)
                continue
            ...
            # Position the text closer to the box
            new_text.next_to(number_box, RIGHT, buff=0.1)  # Reduced buffer to 0.1
            
            # Create a new group with the original box, number, and new text
            new_line_mob = VGroup(number_box, number_text, new_text)
            shorter_lines_bottom_mobs.append(new_line_mob)

        # Create new VGroups with the shorter text
        shorter_lines_top_vg = VGroup(*shorter_lines_top_mobs)
        shorter_lines_bottom_vg = VGroup(*shorter_lines_bottom_mobs)

        # Shift everything to the right
        shift_amount = RIGHT * 4.0  # Adjust this value as needed

        # Replace the original lines with shortened versions and shift them
        self.play(
            FadeOut(lines_top_vg),
            FadeOut(lines_bottom_vg),
            run_time=0.8
        )

        self.play(
            FadeIn(shorter_lines_top_vg),
            FadeIn(shorter_lines_bottom_vg),
            run_time=0.8
        )

        # Shift all relevant elements to the right
        elements_to_shift = VGroup(
            shorter_lines_top_vg, 
            shorter_lines_bottom_vg,
            left_brace, 
            left_brace_bottom,
            arrow_from_brace_top, 
            arrow_from_brace_bottom,
            chunk_size_text_top, 
            chunk_size_text_bottom
        )

        self.play(
            elements_to_shift.animate.shift(shift_amount),
            run_time=1.2
        )
        self.wait(0.5)
        ...
        y_length = 5  # vertical axis length
        x_length = 9  # horizontal axis length
        ...
        # ================== ADVANCED HEATMAP VISUALIZATION SYSTEM ==================
        # Create comprehensive heatmap elements group for sophisticated data visualization
        heatmap_elements = VGroup(
            y_axis, x_axis,           # Main coordinate axes
            y_labels, x_labels,       # Axis labeling systems
            grid_lines               # Grid structure for data organization
        )

        # Create a group of all the elements to fade out
        elements_to_fade = VGroup(
            shorter_lines_top_vg,               # Top segments
            shorter_lines_bottom_vg,            # Bottom segments
        ...
        shift_right = RIGHT * 0  # Adjust this value to control how far right it moves
        ...
        formula_x = anchor_x + x_length + 2.2  # Move slightly further right
        ...
        cosine_values = [
            [0.77, 0.80, 0.45, 0.20, 0.22, 0.15, 0.39, 0.33, 0.31, 0.07, 0.63, 0.39, 0.14, 0.07, 0.17, 0.09, 0.48, 0.33],
            [0.33, 0.46, 0.92, 0.56, 0.22, 0.17, 0.19, 0.25, 0.24, 0.08, 0.33, 0.19, 0.14, 0.14, 0.20, 0.14, 0.12, 0.26],
            [0.19, 0.30, 0.29, 0.17, 0.87, 0.34, 0.09, 0.11, 0.19, 0.20, 0.22, 0.15, 0.10, 0.08, 0.16, 0.17, 0.07, 0.12],
            [0.51, 0.58, 0.20, 0.24, 0.10, 0.19, 0.78, 0.69, 0.21, 0.17, 0.47, 0.53, 0.15, 0.24, 0.22, 0.20, 0.47, 0.53],
            [0.31, 0.21, 0.30, 0.14, 0.29, 0.16, 0.15, 0.11, 0.74, 0.60, 0.16, 0.13, 0.12, 0.13, 0.21, 0.14, 0.22, 0.10],
            [0.56, 0.63, 0.32, 0.25, 0.20, 0.19, 0.45, 0.34, 0.17, 0.14, 0.87, 0.63, 0.15, 0.28, 0.19, 0.18, 0.41, 0.32],
            [0.13, 0.17, 0.16, 0.13, 0.06, 0.23, 0.23, 0.23, 0.10, 0.11, 0.14, 0.29, 0.78, 0.53, 0.10, 0.18, 0.12, 0.13],
            [0.21, 0.19, 0.19, 0.23, 0.20, 0.30, 0.26, 0.22, 0.13, 0.27, 0.16, 0.30, 0.11, 0.24, 0.85, 0.77, 0.15, 0.20],
            [0.57, 0.42, 0.14, 0.23, 0.08, 0.16, 0.59, 0.43, 0.28, 0.13, 0.47, 0.46, 0.12, 0.20, 0.20, 0.20, 0.86, 0.54]
        ...
        # NEW CODE: Scale down the heatmap before adding the box and text
        # First, create a group with all heatmap elements (grid, axes, labels, values)
        all_heatmap_elements = VGroup(
            heatmap_elements,  # Contains y_axis, x_axis, y_labels, x_labels, grid_lines
            *[col_values for col_values in column_values]  # Add all the cell values
        )

        # Define the center point around which to scale
        scale_center = anchor + RIGHT * (x_length/2) + UP * (y_length/2)  # Center of heatmap
        scale_factor = 0.7
        shift_amount = 1

        # Animate scaling down the heatmap (adjust scale factor as needed)
        self.play(
            all_heatmap_elements.animate.scale(scale_factor, about_point=scale_center).shift(LEFT * shift_amount),
            run_time=1.2
        )
        self.wait(0.5)

        # CALCULATE CORRECT POSITIONS FOR THE BRACE AFTER SCALING AND SHIFTING
        # The x-coordinates need to be:
        # 1. Scaled by scale_factor
        # 2. Adjusted for the scaling's effect (depends on where the scale center is)
        # 3. Shifted left by shift_amount

        # Get the scaled positions of where the x-axis now begins and ends
        # This uses the x_axis object that's now part of the scaled group
        scaled_x_axis = all_heatmap_elements[0][1]  # This should be the x_axis object
        brace_start_x = scaled_x_axis.get_start()[0]
        brace_end_x = scaled_x_axis.get_end()[0]
        brace_y = scaled_x_axis.get_start()[1] - 0.5  # Position below the axis

        # Create the horizontal curly brace using the scaled positions
        x_brace = Brace(
            Line(
        ...
        brace_text = Text("Column = 18", font_size=16, color=WHITE)
        ...
        # Also adjusted for scale and shift
        text_x = brace_end_x + 0.5  # Position to the right of the scaled heatmap
        text_start_y = anchor_y + y_length - -0.7  # Start at the top

        # Define the position for the rectangular box on the right side
        right_side_x = brace_end_x + 0.9  # Position to the right of the scaled heatmap
        ...
        # Get the scaled positions for grid calculations
        scaled_x_axis = all_heatmap_elements[0][1]  # This is the x_axis after scaling
        scaled_y_axis = all_heatmap_elements[0][0]  # This is the y_axis after scaling

        # Get the new anchor and dimensions after scaling
        new_anchor_x = scaled_x_axis.get_start()[0]
        new_anchor_y = scaled_x_axis.get_start()[1]
        new_x_length = scaled_x_axis.get_length()
        new_y_length = scaled_y_axis.get_length()

        # Recalculate dx and dy based on the new dimensions
        new_dx = new_x_length / num_cols
        new_dy = new_y_length / num_rows

        # 1. Identify the highest value in each column - using new coordinates
        ...
        for j in range(num_cols):
            ...
            for i in range(num_rows):
                if cosine_values[i][j] == max_value:
                    # Calculate the cell position with the NEW coordinates
                    cell_x = new_anchor_x + (j + 0.5) * new_dx
                    cell_y = new_anchor_y + (i + 0.5) * new_dy
                    max_value_positions.append((i, j))
                    
                    # Create a red background with the correct size
                    cell_bg = Rectangle(
                        width=new_dx * 0.9,
                        height=new_dy * 0.9,
                        color=RED,
                    ...
                    # Use the correct font size for the scaled text
                    value_text = Text(f"{max_value:.2f}", font_size=14*scale_factor, color=WHITE, weight=BOLD)
                    value_text.move_to([cell_x, cell_y, 0])
        ...
        kept_values = [
            ...
            (8, 7, 0.43),    # 8th column
            (0, 8, 0.31),    # 9th column
            (2, 9, 0.20),    # 10th column
            (0, 10, 0.63),   # 11th column
            (3, 11, 0.53),   # 12th column
            (3, 12, 0.15),   # 13th column
            (5, 12, 0.15),   # 13th column
            (5, 13, 0.28),   # 14th column
            (3, 14, 0.22),   # 15th column
            (3, 15, 0.20),   # 16th column
            (0, 16, 0.48),   # 17th column
            (3, 17, 0.53)    # 18th column
        ]

        # Similarly adjust the 2nd best matches
        kept_values_rectangles = VGroup()
        kept_values_text = VGroup()

        for row, col, val in kept_values:
            # Calculate the cell position with NEW coordinates
            cell_x = new_anchor_x + (col + 0.5) * new_dx
            cell_y = new_anchor_y + (row + 0.5) * new_dy
            
            # Create a rectangle around the cell with correct size
            ...
            # Create text for the kept value with scaled font
            value_text = Text(f"{val:.2f}", font_size=14*scale_factor, color=YELLOW, weight=BOLD)
            value_text.move_to([cell_x, cell_y, 0])
        ...
        # Stack the two tokens HORIZONTALLY and position them below the heading
        legends = VGroup(best_match_legend, second_match_legend)
        legends.arrange(RIGHT, buff=0.8)  # Horizontal arrangement with more space between

        top_y = new_anchor_y + new_y_length + 0.8  # Add a small buffer
        center_x = new_anchor_x + new_x_length/2  # Center of the heatmap horizontally

        # Position the legends
        legends.move_to([center_x, top_y, 0], aligned_edge=DOWN)
        ...
        # ------------------------------------------------------------
        # ------------------------------------------------------------
        # Horizontal curly braces pointing UP for paired best matches (columns 1-2, 3-4, etc.)
        # ------------------------------------------------------------
        paired_best_match_braces = VGroup()

        # Group columns in pairs and create braces for each pair
        for pair_start in range(0, num_cols, 2):  # Step by 2 to get pairs 0-1, 2-3, 4-5, etc.
            # Make sure we don't go out of bounds
            if pair_start + 1 >= num_cols:
                continue
            
            # Find the best match rows for both columns in this pair
            best_row_1 = next((r for r, c in max_value_positions if c == pair_start), None)
            best_row_2 = next((r for r, c in max_value_positions if c == pair_start + 1), None)
            
            if best_row_1 is not None and best_row_2 is not None:
                # Calculate the horizontal span for both columns
                start_x = new_anchor_x + pair_start * new_dx
                end_x = new_anchor_x + (pair_start + 2) * new_dx  # +2 to include the full width of both columns
                
                # Position the brace just above the cells (closer to the boxes)
                brace_y = new_anchor_y + (min(best_row_1, best_row_2) + 1) * new_dy + 0.05
                
                # Create the horizontal brace spanning both columns, pointing UP
                brace = Brace(
                    Line(
                        start=[start_x, brace_y, 0],
                        end=[end_x, brace_y, 0]
                    ),
                    direction=UP,  # Changed to UP instead of DOWN
                    buff=0.05,     # Reduced buffer to position closer to boxes
                    color=WHITE
                )
                
                paired_best_match_braces.add(brace)

        # Animate all horizontal paired braces
        self.play(Create(paired_best_match_braces), run_time=1.0)
        self.wait(0.5)

        # Create and animate a clear L-shaped arrow and "Mapping Window" text for one of the braces
        # Use the third brace (columns 4-5) based on the screenshot
        target_brace = paired_best_match_braces[7]  # Adjust index as needed to match desired brace position
        ...
        # Create just the "Mapping Window" text in yellow (no box)
        ...
        # --- pulsing effect for horizontal paired braces -----------------
        pulse = AnimationGroup(
            *[Indicate(
            ...
            ) for b in paired_best_match_braces],
            lag_ratio=0.2,  # slight delay between braces
        )

        # loop the pulse twice
        ...
        text4 = Text("This is called Mapping Window = Slope = 9/18", font_size=12, color=WHITE)
       ...
       # After the fade out is done
        ...
        for i, j in max_value_positions:
            # Calculate the cell position using NEW coordinates
            cell_x = new_anchor_x + (j + 0.5) * new_dx  # Use scaled coordinates
            cell_y = new_anchor_y + (i + 0.5) * new_dy  # Use scaled coordinates
